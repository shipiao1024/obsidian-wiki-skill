...
- 覆盖改善报告：新增了哪些域/问题的来源
- 遺留缺口：仍然欠覆盖的域
- 建议下一步：是否有值得 deep-research 的战略级问题（autoresearch 完成后可建议，与入库完成输出不同）
...
|------|-------------|--------------|---------------|
| 阶段数 | 3 | 3 | 8 |
| 驱动力 | 缺口驱动（盲区→搜索） | 问题驱动（快速假说→验证） | 推理驱动（LLM 先推理→联网扩展→vault 补充） |
| 分析框架 | 无 | 无 | 横纵双轴（纵向时间轴 + 横向竞品轴） |
| 搜索轮数 | 3 轮递进 | 2 轮定向 | 按推理线程并行 |
| 产出 | 覆盖改善报告 | 搜索指南+简要结论 | 叙事驱动结构化报告（纵向/横向/交汇） |
