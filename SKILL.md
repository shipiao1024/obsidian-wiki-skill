---
name: obsidian-wiki
description: 意图驱动的知识生长系统。入库时编译围绕意图，领域从内容涌现，跨库自动桥接洞察。
version: "2.1"
---

# Obsidian Wiki Skill

知识库随意图生长。你说为什么关注，系统围绕意图组织认知；新领域反复出现，系统主动感知并提议；查一个库，自动桥接其他库的洞察。

你（Claude Code）是主入口和决策者，Python 脚本处理机械操作。

## 三层架构

```
意图层   Value Point  — purpose.md 价值锚点，表达"为什么关注"
涌现层   Domain + Proposal — domain-proposals.json，领域从内容涌现
物理层   Vault — raw/ 不可变证据 + wiki/ AI 编译知识层
```

- **Value Point ≠ 领域标签**。标签说"关注什么"，Value Point 说"为什么关注"
- **领域从内容涌现**，不需要预先声明。≥3 篇来源触发提案
- **跨 Vault 桥接**。检索时自动从匹配的其他 vault 补充洞察

## 触发条件

**硬触发**（始终激活）：
- 用户消息含入库 URL：`mp.weixin.qq.com`、`bilibili.com/video`、`youtube.com/watch`、`youtu.be/`、`douyin.com/video`
- 用户消息含意图关键词：`落盘`、`入库`、`ingest`、`归档`、`save to wiki`、`存到知识库`

**软触发**（vault 上下文存在时激活）：
- 用户消息含：`知识库`、`wiki`、`维护知识库`、`日常维护`、`lint`、`review`、`sweep`、`状态`、`维护建议`
- 用户说 `autoresearch`、`自动研究`、`深入研究`、`deep research`、`深度分析`、`系统分析`
- 用户说 `沉淀`、`结晶`（洞见沉淀流程）

**不触发**：
- 用户只要求总结/翻译/解释文章，无入库意图
- 用户问与知识库无关的通用问题
...
| **维护** | "lint" / "体检" / "日常维护" / "review" / "sweep" / "状态" | `references/maintenance-guide.md` |
| **沉淀** | "沉淀" / "结晶" | `references/interaction.md` § 洞见沉淀 |
| **保存对话** | "save" / "保存对话" / "记录讨论" | `references/interaction.md` § 保存对话 |
| **首次配置** | 首次调用或依赖缺失 | `references/setup.md` |

## 通用约定

### Vault 路径
- 从 `vault.conf` 或用户指定的 `--vault` 获取
- 多 vault 时按 `purpose.md` Value Point + 关注领域匹配自动选择

### 写入边界
- **你直接写**：`wiki/outputs/`（查询结果、delta 草稿）→ 临时工作产物，不污染主知识层
- **脚本写的**：`raw/`、`wiki/briefs/`、`wiki/sources/`、`wiki/concepts/` 等正式页面 → 通过入库流水线
- **不主动修改**：正式 wiki 页面（除非用户明确要求）

### 引用规范
- 用 `[[页面名]]` 格式引用 wiki 页面
- 区分 vault 知识（标注来源）和模型补充知识（标注"模型知识，vault 中暂无"）
...
查询相关：wiki_query.py（索引重建+结果写入，查询逻辑由你执行）、wiki_index_v2.py（语义索引构建+查询）、wiki_retrieve.py（智能检索，含跨Vault补充，替代grep搜索）
...
Vault路由：reroute_vault.py（Source迁移工具）
...
脚本详情见：PowerShell 示例：加载对应 guide 文件后参考。
...
| 文件 | 内容 | 加载时机 |
|------|------|---------|
| `references/ingest-quickstart.md` | 入库快速指南（精简版，核心流程） | 首次入库 / 快速参考 |
| `references/ingest-guide.md` | 入库工作流完整版（fetch → ingest → compile → apply → report） | 入库任务详细参考 |
| `references/query-guide.md` | 查询工作流（智能检索 → 理解 → 合成 → 写入） | 查询任务 |
| `references/research-guide.md` | 深度研究工作流（推理驱动 8 阶段 + 横纵双轴分析） | 深度研究 |
| `references/maintenance-guide.md` | 维护工作流（脚本收集 → LLM 判断 → 脚本执行） | 维护任务 |
| `references/interaction.md` | 用户对话路由 + 入库后引导 + 保存对话 | 保存对话 / 入库引导 |

### LLM 约束 Prompt（guide 引用）
| 文件 | 内容 | 引用方 |
|------|------|--------|
| `references/prompts/lint_semantic.md` | 健康检查语义分析约束 | maintenance-guide |
| `references/prompts/claim_evolution.md` | 主张关系分析约束 | maintenance-guide |
| `references/prompts/synthesis_refresh.md` | 综合页内容生成约束 | maintenance-guide |
...
| `references/prompts/insight_detection.md` | 洞见识别约束（10 信号加权评分，阈值 ≥ 3） | query-guide |
...
| `references/prompts/query_synthesis.md` | 查询综合与验证约束 | query-guide |
| `references/prompts/research_hypothesis.md` | 研究假说形成与校准约束 | research-guide |
