...
class SourceAdaptersTests(unittest.TestCase):
    ...
    def test_run_wechat_adapter_returns_ok_result(self) -> None:
        ...
        with mock.patch.object(_wechat_mod.subprocess, "run") as run_mock:
    ...
    def test_run_video_adapter_reports_empty_result_without_subtitle(self) -> None:
        ...
        with mock.patch.object(_video_mod, "resolve_video_adapter_command", return_value=["yt-dlp"]), mock.patch.object(_video_mod.subprocess, "run") as run_mock:
            result = source_adapters.run_video_adapter(
                source_id="video_url_bilibili",
                input_value="https://www.bilibili.com/video/BV1xx",
                work_dir=work_dir,
    ...
    def test_expand_video_collection_urls_retries_without_cookies_on_winerror_10013(self) -> None:
        payload = '{"entries":[{"id":"BV1xx"}]}'
        ...
        with mock.patch.object(_collection_mod, "resolve_video_adapter_command", return_value=["yt-dlp"]), mock.patch.dict(
            ...
            _collection_mod.subprocess,
            "run",
            side_effect=[cookie_error, mock.Mock(stdout=payload, stderr="")],
        ) as run_mock:
            urls = source_adapters.expand_video_collection_urls(
                source_id="video_playlist_bilibili",
                input_value="https://www.bilibili.com/list/example",
        ...
        self.assertEqual(urls, ["https://www.bilibili.com/video/BV1xx"])
        first_cmd = run_mock.call_args_list[0].args[0]
        second_cmd = run_mock.call_args_list[1].args[0]
        self.assertIn("--cookies", first_cmd)
        self.assertNotIn("--cookies", second_cmd)

    def test_expand_video_collection_urls_reports_browser_cookie_copy_failure_actionably(self) -> None:
        error = subprocess.CalledProcessError(
            1,
            ["yt-dlp"],
            stderr="ERROR: Could not copy Chrome cookie database. See https://github.com/yt-dlp/yt-dlp/issues/7271 for more info",
        ...
        with mock.patch.object(_collection_mod, "resolve_video_adapter_command", return_value=["yt-dlp"]), mock.patch.dict(
            ...
            with self.assertRaisesRegex(RuntimeError, "WECHAT_WIKI_VIDEO_COOKIES_FILE"):
                source_adapters.expand_video_collection_urls(
                    source_id="video_playlist_bilibili",
                    input_value="https://www.bilibili.com/list/example",
                    work_dir=ROOT / ".tmp-tests" / "playlist-bilibili-browser-cookie-fail",
                )

    def test_expand_video_collection_urls_falls_back_to_cookie_file_when_browser_cookie_copy_fails(self) -> None:
        browser_cookie_error = subprocess.CalledProcessError(
            1,
            ["yt-dlp"],
            stderr="ERROR: Could not copy Chrome cookie database. See https://github.com/yt-dlp/yt-dlp/issues/7271 for more info",
        ...
        with mock.patch.object(_collection_mod, "resolve_video_adapter_command", return_value=["yt-dlp"]), mock.patch.dict(
            ...
            _collection_mod.subprocess,
            "run",
            side_effect=[browser_cookie_error, mock.Mock(stdout=payload, stderr="")],
        ) as run_mock:
            urls = source_adapters.expand_video_collection_urls(
                source_id="video_playlist_bilibili",
                input_value="https://www.bilibili.com/list/example",
                work_dir=ROOT / ".tmp-tests" / "playlist-bilibili-browser-cookie-fallback",
            )
