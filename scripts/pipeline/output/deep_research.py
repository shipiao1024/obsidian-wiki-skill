"""Mode: deep-research — vault briefing + host-agent-driven protocol."""
...
def build_deep_research_output(
    ...
    """Deep research: vault briefing + instructions for host-agent-driven protocol.

    This mode does NOT execute the research itself (host-agent-first principle).
    It produces a briefing of existing vault knowledge and a protocol checklist
    that the host agent follows to orchestrate the 9-phase research cycle.
    """
    ...
    lines.append("> 此模式需要宿主 Agent 执行 9 阶段假说驱动研究协议。")
    ...
    lines.append("2. **需求审计 + 假说形成**：形成 2-4 个可证伪假说，每个创建 question 页追踪")
    lines.append("3. **Vault 证据收集**：对每个假说收集 vault 中已有的确认/反驳证据")
    lines.append("4. **假说驱动联网研究**：逐轮搜索验证假说，更新依赖账本和置信度")
    lines.append("5. **外部事实校准**：产出共识/边界/争议/假说结果四块")
    lines.append("6. **根本问题挖掘**：压缩到 1-3 根本问题")
    lines.append("7. **情景压力测试**：3-4 情景 × 每个结论，提取边界条件")
    lines.append("8. **预验尸**：3 个失败情景映射账本根节点")
    lines.append("9. **收敛与报告打包**：证据密度检查 → Why/What/How/Trace 报告")
    lines.append("")
