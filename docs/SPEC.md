...
wiki/             ← AI 编译知识层（可演化）
  ...
  research/       ← 深度研究报告 + PDF（纵向/横向/交汇 结构）
...
```
URL/文件/文本 → [1. Fetch + 格式归一化] → [2. Ingest(raw)] → [3. Two-Step CoT Compile] → [4. Risk-Graded Apply(wiki)] → [5. Review + 触发检测]
```
...
| Apply | 编译结果 | wiki 全层 + 原子卡片 + 深度研究触发 | `pipeline/apply.py` + `pipeline/risk_approval.py` + `pipeline/atomic_cards.py` |
| Review | vault 状态 | lint + review queue + 深度研究建议 + Brief PDF | `wiki_lint.py` + `deep_research_triggers.py` + `pdf_utils.py` |
...
| `deep_research.py` | 深度调研编排（推理驱动 8 阶段：初始化→联网研究→横纵分析→报告打包） |
| `dependency_ledger.py` | 依赖账本 CRUD + 置信度传播 + 充分性门控（研究完成后补充验证） |
| `ingest_report.py` | 入库报告 + 对话式引导（`format_ingest_dialogue()`） |
| `claim_evolution.py` | 主张演化追踪（reinforce / contradict / extend 关系，依赖 wiki_lint） |
| `pdf_utils.py` | **V2.0** PDF 生成工具（Obsidian 语法清理 + 调用 md_to_pdf，brief/report 带封面页），详见 `references/pdf-output.md` |
| `md_to_pdf.py` | Markdown → PDF 转换（Playwright + Chrome 渲染，3 主题 CSS，由 pdf_utils.py 内部调用） |
| `encoding_fix.py` | Windows 控制台 UTF-8 编码修复（5 个主入口脚本已集成） |
| `validation_protocol.py` | 深度报告质量门控（5 项检验：叙事完整性、决策逻辑、反面证据、证据标签、边界条件） |
...
**核心原则**：LLM 先推理、联网扩展，vault 作为补充验证。横纵双轴分析：纵向追时间深度（起源→演进→决策逻辑），横向追同期广度（竞品对比→生态位→趋势判断）。

```
Phase 0: 激活与对象识别（vault 覆盖快照 + 对象类型判断）
Phase 1: 意图扩展（挖掘真实问题）
Phase 2: 假说形成（1-2 个纵深可证伪假说）
Phase 3: 联网信息收集（纵向线 + 横向线 + 补充线，并行）
Phase 4: 纵向分析（起源→演进→决策逻辑→阶段划分）
Phase 5: 横向分析（竞品对比→生态位→趋势判断）
Phase 6: 横纵交汇洞察 + 三剧本推演
Phase 7: Vault 补充验证
Phase 8: 收敛与报告打包 + 5 项质量门控 + PDF 生成
```

**5 项质量门控**：叙事完整性、决策逻辑、反面证据、证据标签、边界条件。结果作为附录写入报告末尾。
...
GitHub 发布包默认不包含 `tests/` 目录。完整测试套件保留在开发工作副本中，用于开发阶段验证源适配器、流水线核心、页面构建器、验证门控、风险审批、原子卡片、深度研究触发、间隔复习、图层层和格式归一化路由。
